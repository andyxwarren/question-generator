 ...
 *
 * Module: N03_Y6_NPV - "Determine the value of each digit in numbers up to 10,000,000"
 */
...
export function generateQuestion(params, level) {
    ...
    switch(operation) {
        ...
        case 'complex_place_value':
            return generateComplexPlaceValue(params, level);
        default:
            return generateIdentifyDigit(params, level);
    }
}
...
/**
 * "Break down [number] into place values"
 */
...
/**
 * Complex multi-step place value questions
 */
function generateComplexPlaceValue(params, level) {
    const questions = [
        () => {
            const num = randomInt(params.min_value, params.max_value);
            const place = randomChoice(params.places.slice(1)); // Not ones
            const digit = getDigitAtPlace(num, place);
            const value = getPlaceValue(num, place);

            return {
                text: `In ${formatNumber(num)}, the digit ${digit} is in the ${place} place. What would the number be if this digit was doubled?`,
                type: 'text_input',
                answer: (num - value + (value * 2)).toString(),
                hint: `Double the value of the ${digit} in the ${place} place`
            };
        },
        () => {
            const num = randomInt(params.min_value, params.max_value);
            const place1 = randomChoice(params.places);
            const place2 = randomChoice(params.places.filter(p => p !== place1));
            const digit1 = getDigitAtPlace(num, place1);
            const digit2 = getDigitAtPlace(num, place2);
            const value1 = getPlaceValue(num, place1);
            const value2 = getPlaceValue(num, place2);
            const sum = value1 + value2;

            return {
                text: `In ${formatNumber(num)}, what is the sum of the values of the digits in the ${place1} place and the ${place2} place?`,
                type: 'text_input',
                answer: sum.toString(),
                hint: `Add the values from both places`
            };
        }
    ];

    const questionFunc = randomChoice(questions);
    const q = questionFunc();
