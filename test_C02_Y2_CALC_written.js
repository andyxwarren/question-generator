/**
 * Test Suite for C02_Y2_CALC_written Generator
 * Validates Schema v2.0 compliance for Year 2 written calculation module
 */

import C02_Y2_Generator from './src/generators/C02_Y2_CALC_written.js';
...
function validateSchemaV2(question, testName) {
    ...
    if (question.values) {
        for (const [key, value] of Object.entries(question.values)) {
            // Allow strings for name, item, and similar context values
            if (typeof value === 'string' && !['name', 'name1', 'name2', 'item'].includes(key)) {
    ...
    if (question.type === 'text_input' && typeof question.answer === 'string') {
        errors.push(`${testName}: text_input answer should be a number, not a string`);
    }

    return errors;
}

function testC02_Y2(generator, moduleId) {
    console.log('\n=== Testing C02_Y2_CALC_written ===');
    ...
    for (const level of levels) {
        ...
        for (let i = 0; i < questionsPerLevel; i++) {
            const testName = `C02_Y2_L${level}_Q${i+1}`;
...
function runAllTests() {
    console.log('╔═══════════════════════════════════════════════════════════════╗');
    console.log('║     Testing C02_Y2_CALC_written Generator (Schema v2.0)     ║');
    console.log('╚═══════════════════════════════════════════════════════════════╝');

    const result = testC02_Y2(C02_Y2_Generator, 'C02_Y2_CALC');
    ...
    console.log(`  C02_Y2_CALC: ${status} - ${result.tests} questions, ${result.errors} errors`);
    console.log('');
    ...
    if (result.errors === 0) {
        ...
        console.log('C02_Y2_CALC_written generator is Schema v2.0 compliant.');
        console.log('All 9 operations validated successfully.');
    } else {
